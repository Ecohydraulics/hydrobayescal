...
% Modified by Sebastian Schwindt to enable underscores of methods

\usetikzlibrary{arrows,calc,shapes.misc,shapes.multipart,backgrounds,fit}
\usepackage{color}

% specific coloring (added by Sebastian)
\definecolor{anthrazit}{RGB}{62,68,76}
\definecolor{blue_bright}{RGB}{0,190,255}
...
\definecolor{gray_lightx}{gray}{0.95}
\definecolor{gray_light}{gray}{0.9}
\definecolor{gray_med}{gray}{0.85}
\definecolor{green_light}{rgb}{0.93,1,0.8}
...
\def\umltextcolor{black!85!white}
\def\umldrawcolor{purple!70!black}
\def\umlfillcolor{blue!10!white}
...
\def\endpgfumlcd@object{%
    ...
    \node[anchor=north,this umlcd style,umlcd style school,rounded corners] (\pgfumlcd@ClassName) at (\pgfumlcd@ClassPos) {%
      \pgfumlcd@ifschool{%
        {\pgfutil@font@bfseries\pgfumlcd@ObjectName}%
      }{%
        \texttt{\pgfutil@font@bfseries\pgfumlcd@ObjectName}%
      } % <-- explicit space
      \pgfumlcd@insertAttributesAndOperations{}%
    };%
  \endpgfumlcd@classAndInterfaceCommon
...
%\def\pgfumlcd@attribute#1{%
 % \pgfumlcd@ifnum{\pgfumlcd@ClassAttributesNum}{=}{0}{%
 %   \pgfumlcd@xdef\pgfumlcd@ClassAttributes{\texttt{\detokenize{#1}}}%
 % }{%
...
%}
\def\pgfumlcd@attribute{\pgfutil@ifnextchar[{\pgfumlcd@attribute@}{\pgfumlcd@attribute@[1]}}
\def\pgfumlcd@attribute@[#1]#2{%
  \advance\pgfumlcd@ClassAttributesNum by 1
  \def\pgfumlcd@virtualoperation{\texttt{\detokenize{#2}}}
  \pgfumlcd@ifnum{0}{=}{#1}{%
    \def\pgfumlcd@virtualoperation{{\pgfutil@font@itshape#2}}%
  }{}%
  \pgfumlcd@ifnum{\pgfumlcd@ClassAttributesNum}{=}{1}{%
    \pgfumlcd@xdef\pgfumlcd@ClassAttributes{\pgfumlcd@virtualoperation}%
  }{%
    \let\pgfumlcd@ClassAttributesOld\pgfumlcd@ClassAttributes
    \pgfumlcd@xdef\pgfumlcd@ClassAttributes{\pgfumlcd@ClassAttributesOld\break\pgfumlcd@virtualoperation}%
  }%
}
...
\def\pgfumlcd@association@[#1]#2#3#4#5#6#7{%
  \draw [umlcd style,#1,dashed,opacity=0.58,line width=0.5mm,-stealth] (#2) -- (#5)
...
\def\pgfumlcd@package#1{%
  \def\pgfumlcd@PackageFit{}%
  \def\pgfumlcd@PackageName{\detokenize{#1}}%
}
\def\endpgfumlcd@package{%
  \pgfonlayer{background}%
    \node[umlcd style,draw,inner sep=0.5cm,fit=\pgfumlcd@PackageFit,color=anthrazit,fill=yellow_warm] (\pgfumlcd@PackageName) {};%
    \node[umlcd style,draw,outer ysep=-0.5,anchor=south west, font=\ttfamily,color=anthrazit,fill=yellow_warm] (\pgfumlcd@PackageName caption) at
