...
# The Matplotlib LinearTriInterpolator class uses masked array as its
# output. I had to update how masked arrays are handled for the mesh
# nodes that were outside of the tin boundary.
#
...
# to perform the triangulation; this creates a numpy masked arry
interpolator = mtri.LinearTriInterpolator(tin, t_z)
m_z = interpolator(m_x, m_y)

# this code was added on Feb 29, 2024, and shows where the mask is
# in the m_z masked array
where_are_NaNs = m_z.mask

# if the interpolated value is shown as missing, simply assign to 
# that mesh node the elevation of the closest tin node
