...
class OpenFOAMController:
    ...
    # Converts all time directories to VTK format using foamToVTK.
    ...
    def extract_fields_from_vtk(self, alpha_threshold: float = 0.5, n_avg_timesteps: int = 1) -> Tuple[Any, Any, Any]:
        ...
        # Collect and sort all available VTK files by timestamp
        internal_files = glob.glob(os.path.join(vtk_dir, "**/internal.vtu"), recursive=True)
        ...
        if internal_files:
            def get_timestamp(path):
                ...
                try:
                    ...
                    return 0.0
            internal_files.sort(key=get_timestamp)
            vtk_files_sorted = internal_files
        else:
            vtk_files = sorted(
                f for f in os.listdir(vtk_dir)
                if f.startswith("internalMesh") and f.endswith(".vtu")
            )
            if not vtk_files:
                raise FileNotFoundError(f"No VTK files found in: {vtk_dir}")
            vtk_files_sorted = [os.path.join(vtk_dir, f) for f in vtk_files]
    ...
    def run_multiple_simulations(
        ...
        for i, params in enumerate(params_to_run):
            ...
            try:
                ...
                print(f"EXCEPTION IN RUN: {e}", flush=True)
                traceback.print_exc()
                if self.nloc > 0:
