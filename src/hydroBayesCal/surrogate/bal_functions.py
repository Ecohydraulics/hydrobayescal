...
class BayesianInference:
    ...
    Computes the likelihood of the observations under the surrogate-model
    predictions, the Bayesian model evidence (BME) and relative entropy (RE),
    and draws a posterior parameter sample.

    Parameters
    ----------
    model_predictions : numpy.ndarray
        Array of shape ``[MC_size, n_observations]`` with the surrogate-model
        predictions.
    observations : numpy.ndarray
        Array of shape ``[1, n_observations]`` with the measured observations.
    error : numpy.ndarray
        Array of shape ``[n_observations]`` with the error/noise variances,
        inserted as-is on the diagonal of the covariance matrix.
    prior : numpy.ndarray, optional
        Array of shape ``[MC_size, n_parameters]`` with the prior parameter
        sets. If ``None``, no posterior parameter set is saved.
    prior_log_pdf : numpy.ndarray, optional
        Array of shape ``[MC_size]`` with the prior log-probabilities of each
    ...
    likelihood : numpy.ndarray
        Prior likelihood values (shape ``[MC_size]``) under a multivariate
        Gaussian distribution.
    cov_mat : numpy.ndarray
        Covariance matrix of shape ``[n_observations, n_observations]`` with the
        variances on the diagonal.
    post_likelihood : numpy.ndarray
    ...
    ELPD : float
        Expected log-predictive density (expected value of the posterior
        likelihoods).
    RE : float
    ...
    * **Rejection sampling** divides all likelihoods by the maximum and accepts
    ...
    def rejection_sampling(self):
        ...
        Generates ``MC`` uniformly distributed random numbers (RN). If the
        normalised likelihood ``likelihood / max(likelihood)`` is smaller than
        the corresponding RN, the prior sample is rejected; the remaining
        samples form the posterior.

        Notes
        -----
        * Generates the posterior likelihood, posterior values and posterior
          density arrays.
        * If ``max(likelihood) == 0`` there is no posterior distribution, or the
...
class SequentialDesign:
    ...
    def run_sequential_design(self, prior_samples=None):
        ...
        if 'bal' in exp_design.exploit_method.lower():  # for BAL and AL methods
            ...
            # Select new TP(s)
            new_tp = all_candidates[sorted_idx[:exp_design.n_new_samples]]
            self.selected_criteria = self.total_score[sorted_idx[:exp_design.n_new_samples]]
            ...
            new_tp_size = self.SM.training_points.shape[0] + exp_design.n_new_samples
            # Sample new TPs
    ...
    def run_al_functions(self, exploit_method, candidates, index, m_error, utility_func):
        ...
        ----------
        exploit_method : str
            Exploitation method. Currently supported: ``"bal"`` (Bayesian
            active learning).
        candidates : numpy.ndarray
            Array ``[mc_size, n_params]`` of candidate parameter sets to
            explore, each scored by the utility function.
        index : array-like
            Indices of the candidate samples within the prior pool.
        m_error : numpy.ndarray
            Array ``[n_obs]`` with the measurement error of each observation.
        utility_func : str
            Name of the utility function / active-learning criterion used to
            score each candidate set.

        Returns
        -------
        numpy.ndarray
            Array ``[n_candidates]`` with the score assigned to each candidate,
            in descending order.
        """
