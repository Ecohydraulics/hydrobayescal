...
def run_bal_model(collocation_points,
    ...
    bayesian_dict = {'N_tp': np.zeros(n_iter + 1), 'BME': np.zeros(n_iter + 1), 'ELPD': np.zeros(n_iter + 1),
                     ...
                     'errors': complex_model.measurement_errors,'post_likelihood': [None] * (n_iter + 1), 'post_logpdf': [None] * (n_iter + 1),'post_loglikelihood': [None] * (n_iter + 1)}
    # SURROGATE MODEL
    ...
    for it in range(0, n_iter + 1):
        ...
        if gp_library == 'skl':
            ...
            if complex_model.num_calibration_quantities == 1:
                ...
                if complex_model.multitask_selection == "variables":
                    ...
                    multi_likelihood_var.noise = 1e-3
        ...
        bayesian_dict['post_likelihood'] = bi_gpe.post_likelihood
        bayesian_dict['post_logpdf'] = bi_gpe.post_logpdf
