...
Thanks for contributing! This guide covers the development setup, coding conventions, the documentation build, and **how versioning and PyPI releases work**.

## Development setup

HydroBayesCal requires **Python >= 3.10** and is tested on CPython 3.10-3.12. No upper version bound is enforced (an upper cap on `requires-python` is a hard, non-overridable install error); the *tested* interpreters are advertised through the Trove classifiers in `pyproject.toml`. It uses a `src` layout and a single `pyproject.toml`; there is no `setup.py`.
...
* When you add or change a numerical-solver binding, **preserve the solver-specific strings and file conventions** (e.g. TELEMAC `.cas`/SELAFIN keywords, OpenFOAM `system/controlDict`, Delft3D-FLOW `.mdf` keywords and NetCDF map output). The Python attribute names are shared across bindings; the software-facing values are not.
...
* **MAJOR** i.e. incompatible API changes.
* **MINOR** i.e. new, backward-compatible functionality (e.g. a new solver binding).
* **PATCH** i.e. backward-compatible bug fixes.
...
When you bump it, also update `release`/`version` in `docs/conf.py` so the documentation header matches.

> **PyPI versions are immutable.** Once `X.Y.Z` is uploaded it can never be re-uploaded or overwritten (even if yanked). Always bump the version for any new release.
...
2. On GitHub, go to **Releases > Draft a new release**, create the tag  `vX.Y.Z`, write release notes, and **Publish release**.
3. The workflow builds the sdist + wheel, runs `twine check`, and publishes to PyPI through the trusted publisher. Watch progress under the **Actions** tab.

That's it; note that there is no manual `twine upload` in the normal flow.
...
This is configured already, but for reference, a maintainer with PyPI access registered a GitHub publisher at
...
Only needed if Trusted Publishing is unavailable (e.g. bootstrapping a new project name). Build as above, then upload with a token:
